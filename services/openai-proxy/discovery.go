...
type Node struct {
	...
	// ModelsByEngine is the per-engine attribution behind Models, restricted to
	// the engines this facade fronts. Models alone cannot say *which* engine
	// holds a model, and this facade needs that: one node can run oMLX and vLLM
	// side by side on different ports, so the routing decision and the local
	// backend choice both depend on the attribution. Nil for a peer that sends
	// none, which callers treat as unknown rather than guessing.
