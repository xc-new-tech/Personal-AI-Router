...
const (
	...
	// workloadEngineUnknown is the engine identifier carried in a workload when
	// the chosen node sent no per-engine attribution, so which of this facade's
	// engines will answer is genuinely not known here. It is deliberately not
	// one of the real engine names: reporting a specific engine we did not
	// determine would put a wrong label on the job card.
	workloadEngineUnknown = "openai"
)
...
// non-inference noise. Every engine behind this facade serves the OpenAI API, so these
...
type Proxy struct {
	...
	// backendMu guards backends, the explicit loopback engines the cluster mTLS
	// ingress forwards to, keyed by engine name. The broker sets/clears each via
	...
	// never be re-routed to a peer.
	//
	// It is a map rather than a single backend because this facade fronts a
	// family of engines (see openaiEngines) that can run side by side on one
	// node at different ports.
...
func (p *Proxy) Run(ctx context.Context) error {
	...
	slog.Debug("subscribing to discovery relay for routing targets", "service", string(noderec.ServiceOpenAI))
	if err := p.codec.Notify(noderec.MethodSubscribe, noderec.SubscribeParams{Services: []noderec.ServiceKey{noderec.ServiceOpenAI}}); err != nil {
...
type candidate struct {
	...
	url      *url.URL
	peerUUID string
	// engine is which of this facade's engines holds the model on that node,
	// or "" when the node carries no per-engine attribution. It travels with
	// the forwarded request (engineHeader) so the receiving node serves from
	...
	for i := range candidates {
		...
		proxy := &httputil.ReverseProxy{
			Director: func(req *http.Request) {
				...
				// Name the engine this candidate was ranked for, so a peer
				// running more than one OpenAI engine serves from the same one
				// rather than re-deriving it. Always overwrite: a client must
				// not be able to steer a peer's engine choice by setting it.
				req.Header.Del(engineHeader)
				if cand.engine != "" {
...
// (self-forward guard): a local advertisement could otherwise
...
func (p *Proxy) resolveCandidates(model string) []candidate {
	...
	add := func(n Node) {
		...
		// Which of this facade's engines holds the model on that node. "" when
		// the node sends no attribution; every consumer below treats that as
		...
		case isSelfTarget(u, selfPort):
			...
			//
			// Which local engine depends on the model: this node may run more
			// than one of them. When attribution cannot name one, fall back to
			// the sole healthy backend rather than guessing between engines.
			var lb *url.URL
			var ok bool
			if engine != "" {
			...
			if !ok {
				slog.Debug("resolveCandidates: no local backend for self",
					"node_id", n.ID, "engine", engine, "model", model)
				return
			}
			u = lb
			isLocal = true
...
func upstreamUnreachableID(nodeID string) string {
	return "openai-proxy:upstream-unreachable:" + nodeID
}
...
// engine port the broker's engine poller registered, not the proxy's listen
// port).
func subscribedToNode(n noderec.DirectoryNode) (Node, bool) {
	svc, ok := n.Services[noderec.ServiceOpenAI]
	if !ok || n.IP == "" {
		return Node{}, false
	}
	...
	return Node{
		...
		// Filter on the models this node serves through the OpenAI-speaking
		// engines only, not the cross-engine union, so a model it serves solely
		// via Ollama isn't accepted as an owner here (falls back to the union
		// for a peer that sends no attribution — see EnginesModels).
		Models:         append([]string(nil), n.EnginesModels(openaiEngines...)...),
		ModelsByEngine: attributionFor(n, openaiEngines),
	}, true
}
